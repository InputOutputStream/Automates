...
class Mot:
    ...
    """def concatenation(self, autre_mot):
        if isinstance(autre_mot, str):
            autre_mot = Mot(autre_mot)
            autre_mot.alphabet_set = self.alphabet_set  # Forcer la compatibilité

        if self.alphabet_set != autre_mot.alphabet_set:
            raise ValueError("Alphabets incompatibles pour la concaténation.")

        return Mot(self.contenu + autre_mot.contenu)"""
    ...
    def concatenation(self, autre):
        """
        Concatène self avec un autre mot ou une chaîne (str),
        en forçant l'alphabet du résultat à celui de self.
        """
        if isinstance(autre, str):
            autre = Mot(autre)
            # Forcer l'alphabet de 'autre' à celui de self
            autre.alphabet_set = self.alphabet_set
        elif isinstance(autre, Mot):
            # Vérifie la compatibilité des alphabets, sinon force
            if self.alphabet_set != autre.alphabet_set:
                autre.alphabet_set = self.alphabet_set
        else:
            raise TypeError(f"Type non supporté pour la concaténation : {type(autre)}")

        # Concaténer les contenus
        nouveau_contenu = self.contenu + autre.contenu
        
        # Créer le nouveau Mot avec le bon alphabet
        nouveau_mot = Mot(nouveau_contenu)
        nouveau_mot.alphabet_set = self.alphabet_set
